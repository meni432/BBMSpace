...
public class Preview implements SurfaceHolder.Callback, TextureView.SurfaceTextureListener {
    ...
    public void takePicturePressed() {
         ...
         * BBMSpace setters
         */
        if (applicationInterface.bbmSpaceModeIsOn()) {
            if (MyDebug.LOG)
                Log.d(TAG, "burst manager inital values");

            long turnOffScreenTimer = applicationInterface.bbmSpaceTurnOffScreenTimerPref();
            // TODO WARNING deal with turn off screen timer.
            long slowBurstIntervalTimer = applicationInterface.bbmSpaceSlowBurstIntervalTimerPref();
            int burstDivider = applicationInterface.bbmSpaceBurstDividerIntervalTimerPref();
            long endTripTimer = applicationInterface.bbmSpaceEndTripIntervalTimerPref();
            long burstInterval = applicationInterface.getRepeatIntervalPref();
            long smsInteval = applicationInterface.bbmSpaceSMSIntervalTimerPref();
            String smsPhoneNumber = applicationInterface.bbmSpaceSMSPhoneNumber();
            burstManager.cleanManager();
            burstManager.setBurstInterval(burstInterval);
            burstManager.setBurstSlowTimerPref(slowBurstIntervalTimer);
            burstManager.setBurstDivider(burstDivider);
            burstManager.setmEndTripTimerPref(endTripTimer);
            burstManager.setSmsSendIntervalNumber(smsInteval);
            burstManager.setSmsBasePhoneNumber(smsPhoneNumber);
        }
                ...
                if (applicationInterface.getVideoLowPowerCheckPref()) {
                    ...
                    class BatteryCheckVideoTimerTask extends TimerTask {
                        public void run() {
                            ...
                            burstManager.setLastBatteySnampshot((long)battery_frac);
                            if (MyDebug.LOG)
    ...
    private void takePhotoWhenFocused() {
        ...
        CameraController.PictureCallback pictureCallback = new CameraController.PictureCallback() {
            ...
            public void onCompleted() {
                ...
                if (applicationInterface.bbmSpaceModeIsOn()) {
                    //battery check and save
                    Intent batteryStatus = getContext().registerReceiver(null, battery_ifilter);
                    int battery_level = batteryStatus.getIntExtra(BatteryManager.EXTRA_LEVEL, -1);
                    int battery_scale = batteryStatus.getIntExtra(BatteryManager.EXTRA_SCALE, -1);
                    double battery_frac = battery_level / (double) battery_scale;
                    burstManager.setLastBatteySnampshot(battery_frac);

                    if (burstManager.continueBurst()) {
                        long timer_delay = burstManager.getBurstInterval();
                        if (timer_delay == 0) {
                        ...
                        if (MyDebug.LOG)
                            Log.d(TAG, "continue burst faild");
                    }
                } else {
    ...
    private void updateParametersFromLocation() {
        ...
        if (camera_controller != null) {
            ...
            if (store_location && applicationInterface.getLocation() != null) {
                ...
                burstManager.setLastLocation(location);
            } else {
                if (MyDebug.LOG)
